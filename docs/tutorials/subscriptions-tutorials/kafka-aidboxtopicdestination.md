...
| Aidbox | Profile URL |
| --- | --- |
| ≥ 2604 | `http://health-samurai.io/fhir/core/StructureDefinition/aidboxtopicdestination-kafkaBestEffortProfile`<br>`http://health-samurai.io/fhir/core/StructureDefinition/aidboxtopicdestination-kafkaAtLeastOnceProfile` |
| < 2604 | `http://aidbox.app/StructureDefinition/aidboxtopicdestination-kafka-best-effort`<br>`http://aidbox.app/StructureDefinition/aidboxtopicdestination-kafka-at-least-once` |
...
Setting `keyByResourceId` to `true` keys each message by the FHIR resource id, so all events for the same resource are delivered to the same partition in order.
...
{% hint style="info" %}
**Multi-replica behavior** — every Kafka destination counter except `messagesQueued` is per-replica and held in memory. In a [highly available](../../deployment-and-maintenance/deploy-aidbox/run-aidbox-in-kubernetes/highly-available-aidbox.md) deployment, calls through a load balancer return whichever replica answered. To get cluster-wide totals, query each pod directly and sum the counters; `messagesQueued` is the only field consistent across replicas. See [Counter semantics in the webhook tutorial](webhook-aidboxtopicdestination.md#counter-semantics) for the full table — the same rules apply to all destination kinds.
{% endhint %}
