...
| `_elements`     | Restricts each exported resource to the listed root-level elements plus mandatory ones (`resourceType`, `id`, `meta`). Filtered resources are tagged with `SUBSETTED` in `meta.tag`. Elements use the form `ElementName` (applies to all resource types) or `ResourceType.ElementName` (applies only to that type). You can combine both forms. Repeat the parameter or use comma-separated values. See [Element filtering](#element-filtering). |
...
The `_elements` parameter limits which root-level elements each exported resource contains. Aidbox keeps three mandatory elements (`resourceType`, `id`, `meta`) regardless of what you request, and tags every filtered resource with `SUBSETTED` in `meta.tag` per the FHIR specification.
