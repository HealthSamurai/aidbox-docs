...
`atv` and `context.patient` are Aidbox-specific claims. The SMART App Launch specification does not define them: it passes the patient context as a `patient` parameter next to the access token in the token response, while Aidbox needs it inside the token itself so that scopes can be enforced no matter which server issued it.
